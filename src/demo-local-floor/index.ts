...
(async () => {
	...
	const init = async () => {
		...
		const onXRFrame: XRFrameRequestCallback = (t, frame) => {
			//
			// parse light estimation
			...
			if (pose && glLayer) {
				...
				render(
					...
					() => {
						for (const m of [origin, ...planted] as Float32Array[])
							cubeRenderer.draw(m);
					},
				);

				//
				// move the model at origin
				{
